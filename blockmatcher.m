...
for ulhc_y = ulhc_y_mcframe : blocksize : brhc_y_mcframe
  ...
  for ulhc_x = ulhc_x_mcframe : blocksize : brhc_x_mcframe
    ...
    % x and y are the position of block in current location
    % xx and yy are the motion compensated positions in previous block
...
MSE = mean(mean((mcframe(y_window, x_window) ...
...
title('Current Pictire with Integer MVs');
axis image;
colormap(gray(256));
hold on;
