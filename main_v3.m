...
% for i = 1 : numframes
    ...
    %MSE_old = fractionalblockmatching_old(previous_pic, current_pic, mvs_x_curr, mvs_y_curr);
    %[MSE, mvs_x, mvs_y ] = fractionalblockmatching(previous_pic, current_pic, mvs_x_curr, mvs_y_curr);
    %MSE2_old(i) = MSE_old;
    [MSE, dx, dy] = fractionalblockmatching_old(previous_pic, current_pic, mvs_x, mvs_y);
    MSE2(i) = MSE;
    PSNR2 = 10*log10((255^2) / MSE2(i));
    fprintf('PSNR for Fractional Block Matching is %d. \n', PSNR2);
    
    MSE_ffmpeg = ffmpeg_mse_calc(previous_pic, current_pic, mvs_x_curr, mvs_y_curr);
    MSE3(i) = MSE_ffmpeg;
...
%     vert_pos = 1 : blocksize : size(mvs_x, 1) * blocksize;
%     vert_pos = vert_pos + blocksize / 2;
%     horz_pos = 1 : blocksize :  size(mvs_y, 2) * blocksize;
...
%     colormap(gray(256));
%     hold on;
%     quiver(horz_pos, vert_pos, mvs_x, mvs_y, 0, 'r-');
% 
...
%     hold on;
%     quiver(horz_pos, vert_pos, mvs_x, mvs_y, 0, 'r-');
%     mvs_x_all_ffmpeg = mvs_x_curr(1 : blocksize/4 : size(mvs_x_curr, 1),...
%                        1 : blocksize/4 : size(mvs_x_curr, 2));
%     mvs_y_all_ffmpeg = mvs_y_curr(1 : blocksize/4 : size(mvs_y_curr, 1),...
%                        1 : blocksize/4 : size(mvs_y_curr, 2));
%     vert_pos1 = 1 : blocksize : size(mvs_x_curr, 1) * blocksize;
%     vert_pos1 = vert_pos1 + blocksize / 2;
%     horz_pos1 = 1 : blocksize : size(mvs_x_curr, 2) * blocksize;
