function [MSE, mvs_x_frac, mvs_y_frac] = fractionalblockmatching_old(previous_pic, current_pic, mvs_x, mvs_y)
...
frame_size_h_scaled_max = size(1 : factor : rows, 2);
    frame_size_w_scaled_max = size(1 : factor : cols, 2);
	Xq = ones(frame_size_h_scaled_max, 1) * (1 : factor : cols);
    Yq = (1 : factor : rows)' * ones(1, frame_size_w_scaled_max);
    frame_inter = interp2(double(previous_pic), Xq, Yq);
...
mvs_x_frac = zeros(nblocks_v, nblocks_h);
mvs_y_frac = mvs_x_frac;
...
for ulhc_y = ulhc_y_mcframe : blocksize : brhc_y_mcframe
  ...
  for ulhc_x = ulhc_x_mcframe : blocksize : brhc_x_mcframe
    ...
    mvs_y_frac(ny, nh) = error(index, 2);
    mvs_x_frac(ny, nh) = error(index, 3);
...
colormap(gray(256));
hold on;
quiver(horz_pos, vert_pos, mvs_x_frac, mvs_y_frac, 0, 'r-');


% plotting of FFmpeg MV's with my vectors
fig_no = fig_no + 1;
figure(fig_no);
image((1 : cols), (1 : rows), current_pic);
...
hold on;
quiver(horz_pos, vert_pos, mvs_x_frac, mvs_y_frac, 0, 'r-');
mvs_x_all = mvs_x(1 : blocksize/4 : size(mvs_x, 1), 1 : blocksize/4 : size(mvs_x, 2));
mvs_y_all = mvs_y(1 : blocksize/4 : size(mvs_y, 1), 1 : blocksize/4 : size(mvs_y, 2));
vert_pos1 = 1 : blocksize : size(mvs_x, 1) * blocksize;
vert_pos1 = vert_pos1 + blocksize / 2;
horz_pos1 = 1 : blocksize : size(mvs_x, 2) * blocksize;
horz_pos1 = horz_pos1 + blocksize / 2;
quiver(horz_pos1, vert_pos1, mvs_x_all, mvs_y_all, 0, 'g-');
hold off;
